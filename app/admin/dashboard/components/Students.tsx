...
export default function Students() {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (!form.studentName || !form.fatherName || !form.classApplying) {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.statsRow}>
        <div className={`${styles.statCard} ${activeClass === "all" ? styles.active : ""}`}
        ...
        <div className={`${styles.statCard} ${activeClass === "active" ? styles.active : ""}`}
                ...
                <tr key={s._id}>
                  ...
                  <td>
                    <div className={styles.actions}>
                      <button className={`${styles.actionBtn} ${styles.viewBtn}`} title="View Details" onClick={() => { setSelectedStudent(s); }}>
                        <FaEye />
                      </button>
                      <button className={`${styles.actionBtn} ${styles.editBtn}`} title="Edit Student" onClick={() => handleEdit(s)}>
                        <FaUserEdit />
                      </button>
                      <button className={`${styles.actionBtn} ${styles.deleteBtn}`} title="Delete Student" onClick={() => setDeleteStudent(s)}>
                        <FaTrash />
                      </button>
                    </div>
                  </td>
          ...
          <div className={styles.formModal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalProfile}>
              <div className={styles.modalPhotoWrapper}>
                {selectedStudent.studentPhoto && !photoErrors[selectedStudent._id] ? (
                  <img src={selectedStudent.studentPhoto} className={styles.modalStudentPhoto} alt={selectedStudent.studentName} onError={() => setPhotoErrors(prev => ({ ...prev, [selectedStudent._id]: true }))} />
                ) : (
                  <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1e3a5f 0%, #0f2744 100%)', color: 'white', fontSize: '36px', fontWeight: 700, borderRadius: '50%' }}>
            ...
            <div className={styles.modalInfoSection}>
              ...
              <div className={styles.modalInfoGrid}>
                <div className={styles.modalInfoItem}><span>Date of Birth</span><p>{selectedStudent.dob || "N/A"}</p></div>
                <div className={styles.modalInfoItem}><span>Gender</span><p>{selectedStudent.gender || "N/A"}</p></div>
                <div className={styles.modalInfoItem}><span>Class</span><p>{selectedStudent.classApplying || "N/A"}</p></div>
                <div className={styles.modalInfoItem}><span>Status</span><p style={{ textTransform: "capitalize" }}>{selectedStudent.status || "active"}</p></div>
              </div>
            </div>
            <div className={styles.modalInfoSection}>
              <h4 className={styles.modalInfoTitle}>Parent Information</h4>
              <div className={styles.modalInfoGrid}>
                <div className={styles.modalInfoItem}><span>Father's Name</span><p>{selectedStudent.fatherName || "N/A"}</p></div>
                <div className={styles.modalInfoItem}><span>Mother's Name</span><p>{selectedStudent.motherName || "N/A"}</p></div>
              </div>
            </div>
            <div className={styles.modalInfoSection}>
              <h4 className={styles.modalInfoTitle}>Contact Information</h4>
              <div className={styles.modalInfoGrid}>
                <div className={styles.modalInfoItem}><span>Email</span><p>{selectedStudent.email || "N/A"}</p></div>
                <div className={styles.modalInfoItem}><span>Phone</span><p>{selectedStudent.phone || "N/A"}</p></div>
              </div>
            </div>
            <div className={styles.modalActions}>
              <button className={styles.modalBtnSecondary} onClick={() => handleEdit(selectedStudent)}><FaUserEdit /> Edit</button>
              <button className={styles.modalBtnPrimary} onClick={() => handleStatusChange(selectedStudent._id, selectedStudent.status === "active" ? "inactive" : "active")}>
            ...
            <form onSubmit={handleSaveEdit}>
              <div className={styles.formGrid}>
                <div className={styles.formGroup} style={{ gridColumn: 'span 2' }}>
                  ...
                  <input type="text" className={styles.formInput} value={form.studentName} onChange={(e) => setForm({...form, studentName: e.target.value})} required placeholder="Enter student name" />
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>Date of Birth</label>
                  <input type="date" className={styles.formInput} value={form.dob} onChange={(e) => setForm({...form, dob: e.target.value})} />
                </div>
                <div className={styles.formGroup}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <select className={styles.formInput} value={form.classApplying} onChange={(e) => setForm({...form, classApplying: e.target.value})} required>
                    <option value="">Select Class</option>
                    {CLASSES.filter(c => c.id !== "all").map(c => (<option key={c.id} value={c.name}>{c.name}</option>))}
                  </select>
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>Father's Name *</label>
                  <input type="text" className={styles.formInput} value={form.fatherName} onChange={(e) => setForm({...form, fatherName: e.target.value})} required placeholder="Enter father's name" />
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>Mother's Name</label>
                  <input type="text" className={styles.formInput} value={form.motherName} onChange={(e) => setForm({...form, motherName: e.target.value})} placeholder="Enter mother's name" />
                </div>
                <div className={styles.formGroup}>
              ...
              <div className={styles.modalActions}>
                <button type="button" className={styles.cancelBtn} onClick={() => setEditStudent(null)}><FaTimes /> Cancel</button>
        ...
        <div className={styles.modalOverlay} onClick={() => setDeleteStudent(null)}>
          <div className={styles.formModal} onClick={(e) => e.stopPropagation()} style={{ width: '420px' }}>
            ...
            <div className={styles.deleteContent}>
              ...
              <p className={styles.deleteText}>You are about to delete <strong>"{deleteStudent.studentName}"</strong> from the database. This action cannot be undone.</p>
            ...
            <div className={styles.deleteActions}>
              <button className={styles.cancelBtn} onClick={() => setDeleteStudent(null)}><FaTimes /> Cancel</button>
              <button className={styles.deleteBtnConfirm} onClick={handleDelete} disabled={deleting}><FaTrash /> {deleting ? "Deleting..." : "Delete Student"}</button>
            </div>
          </div>
        </div>
      )}
